...
call_1 <- paste("https://www.eventbriteapi.com/v3/events/search/?q=beer&location.address=new+york+city&token=", "H5ZTGFKD2N3DJCLBGIQX", sep="")
...
for(i in 2:pages){
  call_next <- paste("https://www.eventbriteapi.com/v3/events/search/?q=beer&location.address=new+york+city&token=", "H5ZTGFKD2N3DJCLBGIQX", "&page=", i, sep="")
...
current_dataframe_name <- paste("df_beer", current_time, ".csv", sep = "")
# save csv files
write.csv(df_clean, current_dataframe_name)

tracker_name <- paste("beer tracker", current_time, ".txt", sep = "")
write.table(tracker, file = tracker_name, sep = " ", row.names = TRUE, col.names = NA)
print("beer events all done!!")
